(() => {
    ...
    (() => {
        ...
        da.init();
        const chatExist = document.querySelector(".page__chat");
        if (chatExist) (function() {
            ...
            const brandColor = window.CMSHQ_SETTINGS?.brandColor || "#4f4f66";
            const brandName = window.CMSHQ_SETTINGS?.brandName || "Hearsay";
            window.CMSHQ_SETTINGS;
            const displayName = window.CMSHQ_SETTINGS?.user?.name || "James Smith";
            const placeholderText = window.CMSHQ_SETTINGS?.placeholderText || "Type a message...";
            const welcomeMessage = window.CMSHQ_SETTINGS?.welcomeMessage || "Hi there! How can we help you?";
            window.CMSHQ_SETTINGS;
            ...
            function appendScript(src, scriptDependency) {
                if (scriptDependency && !window[scriptDependency]) {
                    setTimeout((function() {
                        appendScript(src, scriptDependency);
                    }), 1e3);
                    return;
                }
            ...
            function createAndAppendMessage(text, type, user, createdAt) {
                const message = document.createElement("div");
                message.classList.add("cmshq_message");
                message.classList.add(type);
                message.title = new Date(createdAt).toLocaleString();
                let formattedText = text;
                if (window.linkifyStr) formattedText = linkifyStr(text, {
                    target: "_blank",
            ...
            function getHighContrastTextColor(hexcolor) {
        ...
        const dynemicToc = document.getElementById("dynamic_toc");
        if (dynemicToc) {
            let postContent = document.querySelector(".post__content");
            let headers = postContent.querySelectorAll("h1, h2, h3");
            for (let i = 0; i < headers.length; i++) headers[i].id = i + 1;
            dynemicToc.innerHTML = '<h3 class="byword_contents_title">Contents</h3> <ul class="byword_contents_list"></ul>';
            let list = document.querySelector(".byword_contents_list");
            for (let i = 0; i < headers.length; i++) {
                if (headers[i].tagName === "H1") continue;
